...
if npx --yes osv-scanner --lockfile=package-lock.json --format=json --output=sbom/osv-audit.json; then
  echo "OSV scan completed; review sbom/osv-audit.json" >&2
  exit 0
fi

echo "OSV scan unavailable; generating fallback SBOM only" >&2
